...
assert(timeline:record("player_status", nil, { escape = true }))
...
assert(timeline:finish(11, "hit", { damage = 15,
    classification = { outcome = "hit", label = "受击", tone = "failure" } }))
...
assert(snapshot.last_round.classification.label == "受击")
assert(#snapshot.last_round.events == 7)
assert(snapshot.last_round.events[1].kind == "action_start")
assert(snapshot.last_round.events[7].kind == "result")
