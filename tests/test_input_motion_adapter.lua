...
local armed, retired_reason = trigger_adapter:arm_semantic_trigger("Escape")
assert(not armed and string.find(retired_reason, "retired", 1, true),
    "retired semantic experiment must fail closed")
...
assert(stm_trigger_set_calls == 0 and stm_trigger_clear_calls == 0,
    "retired experiment must never issue semantic writes")
