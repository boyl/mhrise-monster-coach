...
RiseQuestLoader 会先生成原版任务列表，再追加完全自定义任务；本 Mod 随后仅把已登记的陪练 Quest ID 移到所有其他自定义任务之后。因此每个陪练任务保留怪物所属星级，并进入该星级任务列表末尾；同星级有多个陪练任务时，它们按登记顺序共同组成末尾的陪练任务区。
