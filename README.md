...
> 当前状态：源码候选版 `0.49.37-semantic-trigger-retired`。托管语义写入实验已按反证永久关闭；产品手柄支持回到已验证物理绑定与冲突检测，开发自动验收使用安全的正常外部输入。训练场景终态报告现在会自动审核轮次、判定窗、行为树退出和结果证据等级，减少后续怪物与武器扩展的人工验收。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
