...
> 当前状态：开发预览版 `0.49.24-current-binding-read`。轰龙核心闭环已有多项实机证据，但仓库源码可能领先于已安装副本；尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 将已验证的 `snow.StmInputConfig` 实际键鼠/手柄绑定接入自动校准输入计划和快捷键冲突门禁；未单独存储的手柄组合保持不可用，不猜默认按钮。
