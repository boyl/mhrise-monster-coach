...
> 当前状态：源码候选版 `0.49.36-stm-input-hook-window`。`0.49.35` 已验证真实 `StmPlayerInput` 上一次 set/clear 的安全生命周期，但在整个 `UpdateHID` 结束后写入没有形成回避动作。本版把同样的 1 次 set 与 1 次 clear 收窄到原生 `StmPlayerInput.update` 边界：post-hook 按下、下一次 pre-hook 释放，使输入覆盖一个原生更新周期；离线任务、15 帧稳定中立态、退出释放和隐藏实验门禁保持不变。若仍无 `atk.esc_*`，该托管时序路线即停止。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 在已验证的当前猎人 `snow.player.PlayerInput` 实例上，对 `Atk_X`、`Atk_A`、`Atk_R_A`、`Escape` 各执行一次精确 `isDelay(CommandButton2)` 只读查询，并检查调用上限、失败数和对象生命周期。
2. 只有四项查询完整通过后，才单独设计成对按下/释放实验；位集父类的 `set/clear/setDatas` 在本版仍只记录、不调用。
