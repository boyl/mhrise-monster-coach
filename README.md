...
> 当前状态：源码候选版 `0.49.37-semantic-trigger-retired`。真实 `StmPlayerInput` 对象、成对生命周期和原生 update 窗口均已验证安全，但三种托管写入路径都没有形成猎人动作，因此该实验入口已永久失败关闭，不再换帧、延长按住或扩大命令范围。只读实例捕获保留为证据，手柄产品支持回到已验证物理绑定与冲突检测；开发自动验收继续使用安全的正常外部输入。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
