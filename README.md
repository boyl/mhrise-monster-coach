...
> 当前状态：源码候选版 `0.49.43-optional-weapon-response`。轰龙 MVP 聚焦招式、阶段、派生、精选起手、子弹时间、一键重开和通用训练结果；根据武器、替换技、资源与姿态推荐见切、居合或登龙等技能的 Response 已改为默认关闭的可选扩展，不再阻塞核心验收。托管语义写入实验已按反证永久关闭；开发自动化仍保留严格白名单的武器应对研究入口，但普通陪练不依赖它。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 用一个集中训练批次同时采集起手、Native 判定窗、通用受击/回避状态、结果和行为树退出，并由统一分析器自动审核。
