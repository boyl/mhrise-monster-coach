...
> 当前状态：源码候选版 `0.49.49-overlay-probe-surface`。轰龙 MVP 聚焦招式、阶段、派生、精选起手、子弹时间、一键重开和通用训练结果；根据武器、替换技、资源与姿态推荐见切、居合或登龙等技能的 Response 已改为默认关闭的可选扩展，不再阻塞核心验收。集中开发验收已在一次游戏启动、一次任务中自动跑完 8/8 个精选起手，动作/派生结构化分析覆盖缺口为 0。Overlay 现在按真实字体和屏幕高度分配内容预算，并把同一布局快照交给自动分析器；验收期间会在内存中临时开启核心面板，结束后恢复用户设置，不要求手动操作。上一批次报告完成后发生了重复签名的 `D3D12Core.dll / c0000409` 退出，因此整轮运行稳定性仍是发布阻塞项。尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
