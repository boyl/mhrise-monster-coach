...
> 当前状态：开发预览版 `0.49.16-input-correlation-resilience`。轰龙核心闭环已有多项实机证据，但仓库源码可能领先于已安装副本；尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 集中复验太刀校准的语义门禁：输入传输已实机达到 7/7，下一次只验证基于实时节点窗口的见切、特殊纳刀与居合是否命中预期动作节点。
