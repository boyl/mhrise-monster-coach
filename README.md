...
> 当前状态：源码候选版 `0.49.47-behavior-tree-readiness`。轰龙 MVP 聚焦招式、阶段、派生、精选起手、子弹时间、一键重开和通用训练结果；根据武器、替换技、资源与姿态推荐见切、居合或登龙等技能的 Response 已改为默认关闭的可选扩展，不再阻塞核心验收。集中开发验收已在一次游戏启动、一次任务中自动跑完 8/8 个精选起手，覆盖独立、固定派生和条件派生，动作/派生结构化分析覆盖缺口为 0。该批次报告完成后发生了重复签名的 `D3D12Core.dll / c0000409` 退出，因此只能确认核心场景合同通过，整轮运行稳定性仍是发布阻塞项；验收器现已加入批次后稳定观察，防止以后误报。猎人猫车后会通过原生传送重新进入战斗区并继续同一场景；下一项训练只在行为树明确回到 `Normal / Wait / Move` 后放行。尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
脚本会校验并部署当前源码，只启动一次游戏、选择首个存档、发布一次陪练任务并进入战斗区；随后在同一任务中依次执行 8 个精选起手，覆盖独立招式、固定派生和条件派生。每项只跑一轮，失败即停止，不重复发送输入，已经取得的原始报告和分析仍保存在 `artifacts/core_acceptance/<batch-id>/`。全部完成后，`analyze_core_acceptance.py` 统一核对场景顺序、轮次、Native 判定窗、行为树退出和结果证据。武器技能 Response 默认关闭且不参与该门禁，脚本也不会修改装备、替换技或存档。游戏会在批次结束后保持运行，便于只做最后一次视觉检查。
...
1. 用已完成的单启动批处理执行一次无外部修改干扰的集中实机验收，并自动归档 8 个精选起手的统一结论。
2. 只对结构化证据无法判断的 Overlay 结果文字、帧差与低分辨率可读性做一次集中视觉验收。
3. 根据核心批次结果修正真实缺口；现有 8 个入口已经达到实用版下限，不为凑数量继续堆菜单。
