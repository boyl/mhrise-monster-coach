...
> 当前状态：源码候选版 `0.49.30-semantic-trigger-experiment`。`0.49.29` 已实机确认当前猎人 `snow.player.PlayerInput.isDelay(CommandButton2)` 的四项有界读取契约；本版仅为开发探针增加离线陪练任务中的 `Escape` 单命令、单帧 `getTrg()` 位写入和自然释放核验，不接入正常陪练 UI 或快捷键。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
