...
> 当前状态：源码候选版 `0.49.42-training-response-lifecycle`。托管语义写入实验已按反证永久关闭；产品手柄支持回到已验证物理绑定与冲突检测。开发验收现在可在白名单轰龙起手开始后，按本机只读解析的真实键位自动发送一次玩家应对，并要求游戏时间轴反向证明对应状态；不能证明就失败关闭且不重试。自动启动已确认能完成单次存档选择并进入大厅；任务启动前会临时隔离两个已知的旧特效诊断启动器，结束时按 SHA-256 恢复。最小生命周期状态还会逐步记录任务柜台、选任务、出发和载入边界，异常退出时自动归档并清理请求。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 用一个集中训练批次同时采集起手、Native 判定窗、真实玩家应对、结果和行为树退出，并由统一分析器自动审核。
2. 完成 Overlay 结果文字与帧差的一次集中视觉验收；只把无法由结构化证据判断的视觉可读性交给用户。
3. 在已有 8 个精选起手基础上，按价值补充至 8–12 个，并继续展示固定、条件和随机派生，不按 Action 数量堆菜单。
