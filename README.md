...
> 当前状态：开发预览版 `0.49.17-loadout-aware-action-calibration`。轰龙核心闭环已有多项实机证据，但仓库源码可能领先于已安装副本；尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
...
1. 先用只读 `snow.StmInputManager` 契约探针定位本机实际输入绑定；校准计划按当前活动书的替换技自动排除不适用招式，不再用默认侧键或错误的纳刀流派反复试错。
