...
> 当前状态：源码候选版 `0.49.29-player-input-read-contract`。`0.49.28` 已实机确认位集父类修改成员和当前猎人的真实 `snow.player.PlayerInput` 路径；本版只在该实例上执行四个精确 Boolean 查询，尚未开放语义输入写入。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
