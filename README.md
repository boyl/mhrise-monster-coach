...
> 当前状态：源码候选版 `0.49.31-actionable-trigger-gate`。`0.49.30` 已实机确认 `Escape` 单帧触发位只写入一次并在第 2 个 HID 周期自然释放，但触发时猎人仍处于古塔到达动画，没有形成回避动作；本版增加已验证中立动作节点的连续稳定门禁，并只把 `atk.esc_*` 视为预期回避证据。该实验仍不接入正常陪练 UI 或快捷键。轰龙核心闭环已有多项实机证据，但尚未发布面向普通玩家的一键安装包。仅限单人使用，并请先备份存档。
