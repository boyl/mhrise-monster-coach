...
- `action_reader.lua` 是游戏版本变化最大的边界，只调用明确的零参数 getter 或字段。
...
- 完全自定义陪练任务依赖 RiseQuestLoader 在原版同星级任务之后执行追加，因此陪练任务区位于该星级任务列表末尾。
- 同星级只有一个陪练任务时，它就是列表最后一项；同星级有多个陪练任务时，它们共同组成末尾的陪练任务区。
- 当前不为陪练任务区内部顺序增加运行时 Hook。第二个同星级陪练任务落地且确认存在稳定排序需求时，再基于真实运行时证据实现局部排序，避免与 QuestLoader 的原生 Hook 冲突。
