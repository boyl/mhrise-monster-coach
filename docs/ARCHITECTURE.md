...
指定起手菜单由 Controller 生成单一 `training_menu_view_model`，普通 ImGui 菜单和开发探针共同消费。Model 仍只提供训练目录与派生树，View/菜单渲染不重新判断联机、任务、验证状态或训练生命周期。状态集合、操作所有权和恢复不变量见 `TRAINING_MENU_UX.md`；新增怪物只提供目录数据，不新增另一套菜单状态机。
...
开发期训练响应由独立的 `TrainingResponseAcceptance` 策略层把“场景 + 武器 + 轮数 + 起手 Action + 响应语义”收敛成白名单契约，Windows 输入适配器不认识轰龙或训练场景。执行器复用自动导航已经取得的游戏焦点，开始响应后不再次抢焦点；每个运行时 `round_id` 在发送前即写入进程内去重集合，输入异常也不得重试。外部 `.response.json` 只记录发送事实，游戏内 `training_timeline` 才是动作结果事实，离线分析同时消费二者。这个边界允许后续按证据逐个增加武器响应，而不把怪物 ID、业务时机或成功定义塞入通用按键桥。

开发期结果验收的自动预定位遵循同一边界：`dev_probe_controller` 只发布目标距离、实时几何和“尚未开始起手”的状态，Windows 执行器只负责有限移动；距离连续稳定后才调用原有产品训练入口。预定位不会进入场景数据或玩家菜单，预定位期间的掉血也不会进入随后新建的训练轮。猎人离开战斗区时探针回到共享回场流程，避免在准备区继续发送移动。
...
两次 Manager `getTrg()` 单帧实验都未被玩家动作层消费后，该入口停止重复尝试。`0.49.32` 与 `0.49.33` 又分别以零写入证明当前猎人和全局 InputManager 的 GameObject 均不含 `snow.StmPlayerInput`，位置枚举到此停止。最后一层托管调查只读钩取游戏自然执行的 `StmPlayerInput.update`，从 pre-hook 的 `this` 参数捕获真实实例；`0.49.34` 确认其 `Refinput` 与当前玩家输入对象同址，三个精确方法和一次 Boolean 查询通过。`0.49.35` 的 UpdateHID 末尾成对 set/clear 与 `0.49.36` 的原生 update post/pre 成对窗口都安全释放，却都未被玩家动作层消费。因此 `0.49.37` 永久关闭托管语义写入，arm 请求在任何调用前失败；只读捕获保留用于证据和兼容诊断，不再作为产品输入适配器。
