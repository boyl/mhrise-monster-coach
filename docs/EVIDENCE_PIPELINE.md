...
4. 未被怪物包审核的边始终输出为 `observed_next_attack_candidate`，无论出现多少次。
5. 只有静态 Think/FSM 结构与自然运行时证据共同支持后，才允许在怪物包中声明 `fixed` 或 `conditional`。
6. 强制 Action 探针仅验证起手能否安全进入和退出，不用于证明后继关系。
