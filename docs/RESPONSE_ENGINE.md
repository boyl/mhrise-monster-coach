...
1. 只读探针：先对 Player/PlayerData 类型只枚举含武器、替换技、书、槽、翔虫等语义关键词的字段和零参数方法名称，写入 `runtime_player_state_probe.json`；不调用任何新发现的方法。根据真实结果建立精确白名单后，再识别武器类型、当前书、五个替换技、太刀槽/刃色、翔虫和纳刀术。
2. 证据快照：把原始字段与转换后的稳定语义写入有界诊断 JSON，方便一次实机确认。
3. 纯规则：先用构造状态测试上述六种太刀候选及所有不可用原因。
4. Overlay：显示一条当前推荐、最多两条备选及原因；可独立关闭武器建议。
5. 实机门禁：至少覆盖红/蓝书切换、特殊纳刀/神威居合互斥、翔虫 0/1/2、不同刃色、动作不可取消和多段攻击。
