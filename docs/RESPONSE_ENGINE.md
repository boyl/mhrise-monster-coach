...
`Response` 专指根据玩家当前武器、红/蓝替换技书、装备技能、实时资源和动作状态给出的见切、居合、登龙等武器技能建议。它是默认关闭的可选扩展，不属于轰龙 MVP 的发布前置条件；怪物招式、阶段、派生、精选起手、F6、F7 和通用结果时间轴均不得依赖它。

配置 `weapon_response_extension_enabled=false` 时，Model 不运行武器建议规则，View 不显示武器建议或配装摘要；玩家受击、通用防御/回避状态仍可进入训练时间轴。已有太刀实现、知识数据和开发验收工具继续保留，以便核心闭环稳定后独立推进，不因暂缓而删除证据或重写接口。
...
`action_state.evidence` 当前保存只读 MotionFsm2 节点和 `ActStatus` 标签，详细证据等级与实机门禁见 `PLAYER_ACTION_EVIDENCE.md`。`current_action` 在映射完成前必须为 `nil`；原始节点不得在 View 或太刀规则中被直接解释成见切、居合等语义。
...
- “装备了特殊纳刀”不等于“当前已经处于纳刀姿态”。居合只有在只读语义层同时确认 `special_sheathe_combo` 姿态和怪物前摇时才进入 `available`；否则保持 `wait`。
- 公开实现目前只给出神威相关动作族的宽泛保护前缀，没有给出可与其他太刀动作安全区分的神威纳刀精确节点。因此神威居合即使已装备也保持 `wait`，直到运行时获得并审核精确姿态节点；不得用交换技配置代替姿态证据。
- 候选先按 `available > wait > unknown > unavailable` 排序，同一可用性内再按当前阶段排序：攻击判定中优先回避，已确认特殊纳刀姿态优先居合，收招期优先兜割/樱花，普通前摇才考虑见切、刚·气刃斩或水月。
...
3. 证据快照：把原始字段与转换后的稳定语义写入有界诊断 JSON，方便一次实机确认。
   安全模式仍以只读方式采样猎人生命；发生受击时自动把当前 Action、Motion、动画帧/总帧、归一化进度和伤害聚合到 `observed_hit_timing`，不再要求逐招人工抄帧。该证据只能证明“此帧发生过命中”，不能单独证明整个有效判定区间；达到足够样本并与碰撞状态交叉验证前，不驱动高风险反击建议。
