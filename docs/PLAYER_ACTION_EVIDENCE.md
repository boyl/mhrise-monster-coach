...
`0.49.31` 实机确认中立门禁命中 `wait.main` 且连续稳定 15 帧，随后仍只写入 1 次并自然释放，但等待 60 帧也没有任何动作节点变化。由此排除古塔到达动画，并停止 Manager 位集路线；完整证据见 `docs/evidence/ACTIONABLE_TRIGGER_ACCEPTANCE_2026-08-29.json`。公开 REFramework 文档确认 pre/post application entry 的前后时序，MHRice 源码则不包含运行时输入消费逻辑。`0.49.32` 与 `0.49.33` 回到只读层后，分别确认当前猎人和全局 InputManager 的 GameObject 均不含 `snow.StmPlayerInput`；否证见 `docs/evidence/PLAYER_GAMEOBJECT_STM_INPUT_REJECTION_2026-08-29.json`、`docs/evidence/MANAGER_GAMEOBJECT_STM_INPUT_REJECTION_2026-08-29.json`。`0.49.34` 成功只读捕获自然执行的 `StmPlayerInput.update` 实例并确认 `Refinput` 同址；`0.49.35` 的 UpdateHID 末尾成对 set/clear 与 `0.49.36` 的原生 update post/pre 成对窗口都安全释放，却都没有动作节点变化。最终否证见 `docs/evidence/STM_PLAYER_INPUT_HOOK_WINDOW_REJECTION_2026-08-30.json`；`0.49.37` 已让所有后续 arm 请求失败关闭，不再进行托管语义写入。

`0.49.38` 不恢复上述托管写入。它只在开发自动验收中复用已经验证的 Windows 外部输入桥和本机当前绑定：右回旋 Action 26 的活动轮起点出现后，最多发送一次 `dodge`。发送记录写入独立侧车，只有同轮游戏时间轴实际出现 `ActStatus.Escape` 才算响应已关联；错误武器、绑定读取不完整、失焦、重复轮次或只观察到“输入已发送”均失败关闭。该门禁用于降低后续玩家应对证据的采集成本，不进入普通玩家产品 Controller。
